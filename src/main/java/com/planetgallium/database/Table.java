...
public class Table {
	...
	public void insertRecord(Record recordToInsert) {
		...
		String insertRecordQuery = Table.INSERT_RECORD_QUERY.replace("{table_name}", this.getName())
				.replace("{columns}", columns).replace("{values}", values);
		try (Connection connection = dataSource.getConnection();
	...
	/**
	 * Updates an existing record (row). If the fields parameter contains the
	 * primary key field (ex: uuid), it will update its key value to that given in
	 * the fields It will NOT change the primary key column for all database entries
	 * in any way
	 */
	...
	/**
	 * Searches the database for any records with the matching field value. For
	 * example, the field value could be a username "cervinakuy", of which there
	 * should only be 1 record that contains that username value. Alternatively,
	 * could return a list of multiple elements with all matching records. For
	 * example, searching the database with the field name "kills" and value 6 would
	 * return all the records that have 6 kills.
	 *
	 * Do NOT use if fieldToSearchFor is the primary key. In that case, use
	 * getRecord instead
	 */
	...
	public Record getRecord(Field keyToRecord) {
		if (!verifyPrimaryKey(this, keyToRecord))
			return null;
	...
	public void deleteRecord(Field keyOfRecordToDelete) {
		if (!verifyPrimaryKey(this, keyOfRecordToDelete))
			return;
	...
	/**
	 * Converts columns [fields] (ex: uuid, kills, deaths) to string: "uuid VARCHAR,
	 * kills INT, deaths INT" to be used in SQL statements
	 */
	 ...
	 * "kills=16 experience=85" to be used in SQL statements
	 */
